...
angular.module('sbAdminApp').factory('authService', ['$http', '$q', 'localStorageService', 'ngAuthSettings', '$location', function ($http, $q, localStorageService, ngAuthSettings, $location) {
        ...
        $http.post(serviceBase + 'token', data, { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } }).success(function (response) {
            ...
            $http.post(serviceBase + 'api/account/GetUserProfile',data).success(function (res) {
                ...
                for (var i = 0; i < res.length; i++) {
                    _authentication[res[i].Type] = res[i].Value;
                }
                if (_authentication.useRefreshTokens) {
                    localStorageService.set('authorizationData', { token: response.access_token, userName: loginData.userName, refreshToken: response.refresh_token, useRefreshTokens: true, WhiteLabelId: _authentication .WhiteLabelId});
                }
                else {
    ...
    var _fillAuthData = function () {
        ...
        if (authData) {
            ...
            _authentication.WhiteLabelId = authData.WhiteLabelId;
        }
